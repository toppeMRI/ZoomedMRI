%function main_ktCont
% main file for continuous trajectory based on kt points

extype = 3; % 1: uniform excitation; 2: pre-phasing; 3: cube/hockey puck; 4: slice selective pre-phasing;
...
if isBssfp
    %tipangle = 2*tipangle;   % do this by increasing transmit gain (TG += 60)
end
...
if extype == 3
    ...
    if dorect
        ...
        d((25:40)+xoff,(25:40)+yoff, (ceil(size(b0map_simu,3)/2):ceil(size(b0map_simu,3)/2)+2) + zoff) = 1;
        %          d(20:35,20:45,ceil(size(b0map_simu,3)/2-1):ceil(size(b0map_simu,3)/2)+2) = 1; % test
    else
        xr = 1:size(d,1);
        yr = 1:size(d,2);
        [xxtmp,yytmp] = ndgrid(xr-32,yr-32);
        radius = 6/24*size(d,1);
		xoff = 10; yoff = 0;
        circInd = (xxtmp-xoff).^2 + (yytmp-yoff).^2 < radius.^2;
...
[b1,gx,gy,gz,projIm,M_bloch,K_pro] = ktContRF_greedy2(d,b0map,sens, mask,tipangle,n_pe,ishard,simuRange,tol,extype,isBssfp);
...
if isBssfp == 0
    ncycles = 8;
    zthick = 1;
    netarea = 0; % net area of current grad: sum(gx)*1e-6; this value is typically very small compared to crusher
