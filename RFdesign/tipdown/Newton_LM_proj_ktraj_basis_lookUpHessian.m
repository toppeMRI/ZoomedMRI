...
function [k, costRec, nrmse, exetime] = Newton_LM_proj_ktraj_basis_lookUpHessian(k, d, sens,mask,weightIm,beta,niter,fieldmap,tt, xyzRange, ndt, xfov, yfov, zfov)
...
%       HB = 2*real(JB'*W*JB);
   ...
   %%
   costRec(iter) = norm(e)^2 + beta*norm(b)^2;
...
costRec = [cost0 costRec];
nrmse = [nrmse0 nrmse];
exetime = [0 exetime];
% save projectd_LM_exetime costRec nrmse exetime
