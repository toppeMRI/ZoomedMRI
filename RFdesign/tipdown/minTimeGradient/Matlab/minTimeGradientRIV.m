...
excite = 1; % 1: for excitation k-space, 0: for readout, the kspace def is different
if excite == 1
   C = flipud(C);  % for excitation k space
end
...
if 1 %Hao: add leading and trail to gradient wavform 
   nlead = 20; 
   ntrail = 3; 
   gx_lead = linspace(0,g(1,1),nlead); 
   gx_trail = linspace(g(end,1),0,ntrail); 
   gy_lead = linspace(0,g(1,2),nlead); 
   gy_trail = linspace(g(end,2),0,ntrail); 
   gz_lead = linspace(0,g(1,3),nlead); 
   gz_trail = linspace(g(end,3),0,ntrail); 
   g_lead = [gx_lead', gy_lead', gz_lead']; 
   g_trail = [gx_trail', gy_trail', gz_trail']; 
   g = [g_lead; g; g_trail]; 
   C = cumsum(g,1)*dt*gamma; 
end
...
gamma = 4.257;
k = abs(k);
k1 = ds * 1/st*sqrt(gamma^2*smax^2 - abs(k(1))^2*st^4); %Hao: ds * 1/v *beta
