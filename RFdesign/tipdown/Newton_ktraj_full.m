...
function [k,cost] = Newton_ktraj_full(k, d, sens, mask, weightIm,beta)
b = 0; 
alp = 0.0001; 
alp0 = 2; 
...
for iter = 1:50
   ...
   dire = H\[dkx;dky;dkz]; %descending direction
...
%    b = b -alp * db; 
   % get the gradient 
   [dkxEva, dkyEva, dkzEva, dbEva, e, A] = kb_grad(k, b, d, sens, mask,weightIm,beta);
   ...
   cost(iter) = norm(e); 
   printf('%d | %.5g | %.5g | %.3g \n', iter, cost(iter), 2*alp, grad_norm) 
