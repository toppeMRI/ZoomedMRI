function writewav_sos3d(ofname, b1, g, nomflip)

% Hao: modified from function mat2wav(rho,theta,gx,gy,gz,nomflip,ofname,desc)
...
%
% $Id: writewav_sos3d.m,v 1.1 2015/05/22 21:39:26 jfnielse Exp $
...
[paramsint16 paramsfloat] = sub_myrfstat(abs(b1(:)), nomflip);
...
% make length even
b1 = makeevenlength(b1);   % units: Tesla
gx = makeevenlength(gx);   % units: G/cm
gy = makeevenlength(gy);
gz = makeevenlength(gz);

% fix gradient directions (Hao: the following comments are for stfr5.e, not checked here)
...
function [paramsint16, paramsfloat] = sub_myrfstat(b1, nom_fa)
...
max_b1 = 0.2;         % (Gauss) fixed value, waveform is scaled relative to this
if max(rho(:)) > max_b1;
	rho(rho>max_b1) = max_b1;     % be brutal and just clip it
	warning('RF waveform exceeds 0.2 Gauss, clipped');
end
