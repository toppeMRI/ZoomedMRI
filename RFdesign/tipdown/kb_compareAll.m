function [b1,gx,gy,gz,projIm,M_pr,K_rv] = kb_compareAll(d,fieldmap,sens, mask,tipangle,n_pe,ishard,xyzRange,tol,extype,isBssfp,doOpt,iniTraj,rFOV)
% this code is used to generate the batch the figures for joint design
% paper.
...
if ~isvar('doOpt') % make excitation balanced
    doOpt = 1;
end
if ~isvar('iniTraj') % make excitation balanced
    iniTraj = 4;
end
if ~isvar('rFOV') % make excitation balanced
    rFOV = 1;
end
...
if doOpt
    ...
    switch method
        ...
        case 3
            [ks, cost, nrmse, exetime, k_allIter, b_allIter] = InteriorPoint_ktraj_basis(k, d, sens, mask, weightIm,beta,niter,fieldmap,tt,xyzRange,ndt, xfov, yfov, zfov); % work
        case 4
...
Nt = length(gx);
M_pr = parallel_blochCim(0,double(reshape(b1,[Nt,ncoils])),double(gx),double(gy),double(gz),double(sens),double(xyzRange.x),double(xyzRange.y),double(xyzRange.z),dt,double(fieldmap),mask,T1/1000,T2/1000);
